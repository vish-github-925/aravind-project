...
function ContactForm({form, contactData, handleChange, handleSubmit}) {
  return (
    <form ref={form} action="post" className='bg-white h-full flex flex-col space-y-5 items-start border-2 p-10 rounded-s-lg md:w-2/3 w-full'>
        ...
        <Button type='submit' disabled={contactData.firstname.length === 0 || contactData.lastname.length === 0} onClick={handleSubmit} className='rounded mt-10 text-white bg-blue-400 py-5 px-10 hover:cursor-pointer  active:scale-95 transition-colors ease-linear'>Send</Button>
